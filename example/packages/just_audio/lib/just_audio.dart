...
JustAudioPlatform get _pluginPlatform {
  ...
  if (_pluginPlatformCache == null) {
    ...
    // TODO(Author): Remove this hack once https://github.com/flutter/flutter/issues/10437 is implemented.
    try {
...
class AudioPlayer {
  ...
  Future<Duration?> load() async {
    ...
    if (_active) {
      ...
      return _load(await _platform, _audioSource!, initialSeekValues: initialSeekValues);
    } else {
  ...
  void _broadcastSequence() {
    // TODO(Author): update currentIndex first if it's out of range as a result of
...
class _ProxyHttpServer {
  ...
  Future<Uri?> fetchStreamUrl(Uri uri) async {
    try {
      final Dio _dio = Dio(
      ...
      final Response<dynamic> response = await _dio.getUri(uri);
      return Uri.parse(response.data['url'].toString());
    } catch (error) {
      return null;
    }
  }

  Future<Uri> addUriSoundCloudSource(SoundCloudSource source) async {
    final uri = await fetchStreamUrl(source.uri);
...
//TODO(BEN)
class SoundCloudSource extends IndexedAudioSource {
  final Uri uri;
  final Map<String, String>? headers;
  Uri? _overrideUri;

  SoundCloudSource(this.uri, {this.headers, dynamic tag, Duration? duration}) : super(tag: tag, duration: duration);
  ...
  Future<void> _setup(AudioPlayer player) async {
    await super._setup(player);
    await player._proxy.ensureRunning();
    _overrideUri = await player._proxy.addUriSoundCloudSource(this);
  }
...
class LockCachingAudioSource extends StreamAudioSource {
  ...
  Future<IndexedAudioSource> resolve() async {
    ...
    return file.existsSync() ? AudioSource.uri(Uri.file(file.path)) : this;
  }
